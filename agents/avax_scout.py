...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(message)s',
    handlers=[logging.FileHandler(LOG_DIR / 'avax_scout.log'), logging.StreamHandler(sys.stdout)])
logger = logging.getLogger('avax-scout')

AGENT_ID, SIGNAL_DIR, RUNNING = "avax-scout-1", Path(__file__).parent.parent / 'data' / 'signals', True
...
def generate_signal():
    ...
    return {
        ...
        'asset': 'AVAX-USD', 'direction': "LONG" if random.random() > 0.4 else "SHORT",
        'confidence': round(random.uniform(0.68, 0.88), 4),
        'entry_price': round(price, 2), 'stop_loss': round(price * 0.97, 2),
        'take_profit': round(price * 1.06, 2), 'paper_mode': True
    }
