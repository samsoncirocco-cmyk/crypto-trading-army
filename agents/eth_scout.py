...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(message)s',
    handlers=[logging.FileHandler(LOG_DIR / 'eth_scout.log'), logging.StreamHandler(sys.stdout)])
logger = logging.getLogger('eth-scout')

AGENT_ID, SIGNAL_DIR, RUNNING = "eth-scout-1", Path(__file__).parent.parent / 'data' / 'signals', True
...
def generate_signal():
    ...
    return {
        ...
        'asset': 'ETH-USD', 'direction': "LONG" if random.random() > 0.42 else "SHORT",
        'confidence': round(random.uniform(0.70, 0.94), 4),
        'entry_price': round(price, 2), 'stop_loss': round(price * 0.98, 2),
        'take_profit': round(price * 1.04, 2), 'paper_mode': True
    }
